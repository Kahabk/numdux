...
The installer will automatically set up a local virtual environment (`.venv`), install all Python dependencies, and build the frontend assets (if Node.js is installed) or use the pre-built files.

---

### 2. Running Numdux

Once installed, start the application from the root folder:
...
# Or install globally using standard pip
pip install .
```
Now you can start it from anywhere with just:
```bash
numdux
```
...
By default, Numdux runs at `http://127.0.0.1:8000/`.
