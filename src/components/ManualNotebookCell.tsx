...
export const ManualNotebookCell = memo(function ManualNotebookCell({
  cell,
  sandboxName,
  selected,
  onSelect,
  ...
  return (
    <Cell
      ...
      selected={selected}
      onSelect={onSelect}
      onRun={cell.type === "markdown" ? undefined : onRun}
      onDuplicate={onDuplicate}
      onAddBelow={onAddBelow}
      onToggleReport={onToggleReport}
      includeInReport={cell.includeInReport}
      onExport={() => setShowSource(true)}
      onExplain={() => setShowSource(true)}
      output={output}
    >
      <div className="mb-2 flex items-center justify-between gap-2 text-xs text-muted">
        <span>{cell.type === "markdown" ? "Manual content" : "Manual source - AI will not overwrite this cell."}</span>
        <button className="text-action" onClick={onDelete} type="button">Delete</button>
      </div>
      {showSource && (
        cell.type === "markdown" ? (
          <textarea
            className="min-h-28 w-full resize-y border border-line bg-base p-3 text-sm text-ink outline-none focus:border-accent"
            value={cell.source}
            onChange={(event) => onChange(event.target.value)}
            placeholder="Write notebook notes..."
          />
        ) : (
          <Editor
            height="260px"
            defaultLanguage={cell.type}
            value={cell.source}
