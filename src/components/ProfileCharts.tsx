...
export const ProfileCharts = memo(function ProfileCharts({ profile }: { profile: DatasetProfile }) {
  ...
  const missingness = useMemo(
    () => profile.missingness_chart.filter((item) => item.nulls > 0).slice(0, 10),
    [profile.missingness_chart]
  );

  const missingnessBarsData = useMemo(
    () => missingness.map((item) => ({ label: item.column, count: item.nulls })),
    [missingness]
  );
  ...
  return (
    <div className="grid gap-3 lg:grid-cols-2">
      ...
      <ChartFrame
        title={category ? `${category.column} top values` : secondaryNumeric ? `${secondaryNumeric.column} distribution` : "Category distribution"}
        empty={!category && !secondaryNumeric}
      >
      ...
      <ChartFrame title="Missing values by column" empty={!missingness.length} className="lg:col-span-2">
        {missingness.length > 0 && <HorizontalBars data={missingnessBarsData} />}
      </ChartFrame>
    </div>
  );
});

const ChartFrame = memo(function ChartFrame({ title, empty, children, className = "" }: { title: string; empty: boolean; children: React.ReactNode; className?: string }) {
  return (
    <section className={`border border-line bg-base p-3 ${className}`}>
      <div className="mb-2 text-xs text-muted">{title}</div>
      {empty ? (
        <div className="flex h-48 items-center text-xs text-muted">
          {title === "Missing values by column" ? "No missing values detected." : "No profile values available for this chart."}
        </div>
      ) : (
...
const Histogram = memo(function Histogram({ data }: { data: Array<{ label: string; count: number }> }) {
  return (
...
const HorizontalBars = memo(function HorizontalBars({ data }: { data: Array<{ label: string; count: number }> }) {
  return (
    <ResponsiveContainer width="100%" height="100%">
